...
namespace BID.BaseClass
{
    class DataBase : IDisposable
    {
        ...
        private void Open()
        {
            ...
            {
                con = new SqlConnection(Properties.Settings.Default.db_BIDConnectionString);//创建数据库连接对象
            }
        ...
        /// <summary>
        /// 执行SqlCommand（执行数据库中的添加、修改、删除操作）
        /// </summary>
        /// <param name="M_str_sqlstr">SQL语句</param>
        public void getsqlcom(string SQLstr)
        {
            this.Open();    //打开与数据库的连接
            SqlCommand SQLcom = new SqlCommand(SQLstr, con); //创建一个SqlCommand对象，用于执行SQL语句
            SQLcom.ExecuteNonQuery();   //执行SQL语句
            SQLcom.Dispose();   //释放所有空间
            con.Close();    //调用con_close()方法，关闭与数据库的连接
        }
        #endregion
        ...
        {
            this.Open();  //打开与数据库的连接
            SqlDataAdapter SQLda = new SqlDataAdapter(SQLstr, con);  //创建一个SqlDataAdapter对象，并获取指定数据表的信息
